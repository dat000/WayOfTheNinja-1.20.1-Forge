...
public class SetClanCommandProcedure {
	public static String execute(LevelAccessor world, double x, double y, double z, CommandContext<CommandSourceStack> arguments, Entity entity) {
		if (entity == null)
			return "";
		{
			double _setval = DoubleArgumentType.getDouble(arguments, "clanID");
			(new Object() {
		...
		if (entity instanceof Player _player && !_player.level().isClientSide())
			_player.displayClientMessage(Component.literal(("\u00A7aCharacter clan has been changed to \u00A7f" + new java.text.DecimalFormat("##.##").format(((new Object() {
				public Entity getEntity() {
					try {
						return EntityArgument.getEntity(arguments, "name");
					} catch (CommandSyntaxException e) {
			...
			}.getEntity()).getCapability(WayOfTheNinjaForge1201ModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new WayOfTheNinjaForge1201ModVariables.PlayerVariables())).clan))), false);
